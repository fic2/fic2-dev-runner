	...
	return {
	  ...
	  coreos: {
		...
		content: [
		  ...
		  'After=docker.service fleet.service systemd-journal-gatewayd.service',
		  'Requires=docker.service fleet.service systemd-journal-gatewayd.service',
		  '[Service]',
		...
		content: [
		  ...
		  'ExecStartPre=-/usr/bin/docker rm -f PMX_UI',
                  'ExecStartPre=/usr/bin/timeout -k 4s -s KILL 60s /bin/bash -c \'echo "Starting control loop"; until curl -fsS http://localhost:3001/types 1>/dev/null; do echo "Waiting for localhost on port 3001"; sleep 1s; done; echo "Ending control loop"\'',
                  'ExecStart=/usr/bin/docker run --name=PMX_UI --rm=true -v /var/run/docker.sock:/run/docker.sock:rw --link=PMX_API:PMX_API --link=PMX_CADVISOR:PMX_CADVISOR -e CADVISOR_PORT=8080 --expose=3000 -p 3000:3000  devmediafi/panamax-ui:latest',
		...
		content: [
		  ...
		  'After=panamax-api.service panamax-ui.service',
                  'Requires=panamax-api.service panamax-ui.service',
		  '[Service]',
		  'ExecStartPre=-/usr/bin/docker kill PMX_API_NGINX',
		  'ExecStartPre=-/usr/bin/docker rm -f PMX_API_NGINX',
		  'ExecStart=/usr/bin/docker run --name=PMX_API_NGINX --rm=true -t -v /dev/urandom:/dev/random -v /home/core/hub.conf:/etc/nginx/sites-enabled/hub.conf --link=PMX_API:PMX_API --link=PMX_UI:PMX_UI --expose=6001 -p 6001:6001 --expose=6002 -p 6002:6002 cgeoffroy/nginx-auto_cert:latest',
